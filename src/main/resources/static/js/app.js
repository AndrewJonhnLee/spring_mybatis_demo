...
$(function () {
	...
	$('#zclipCopy').zclip({ 
	  ...
	  copy: function(){
	    return $('#zclipCopyText').val(); 
	  }, 
	  afterCopy: function(){
	    $("<span class='red'/>").insertAfter($('#zclipCopy')).text(' 复制成功'); 
	  } 
	}); 
})
...
var add_discount=function(){
    ...
    $('tr#add_button').before('<tr><td><img src="/static/usr/default/images/ico_del.png" onclick="del_discount(this)" align="absmiddle" title="移除" /></td><td><div class="input-group"><span class="input-group-addon">大于</span><input type="text" name="dis_quantity[]" class="form-control input-sm" value=""><span class="input-group-addon">张</span></div></td><td><div class="input-group"><input type="text" class="form-control input-sm" name="dis_price[]" value="" /><span class="input-group-addon">元</span></div></td></tr>');
};

var del_discount=function(id,gid){
       $.get('/goodlist/deldiscount',{id:gid},    		function(data){    	        if(data=='ok'){    	        	    	        	$(id).parent().parent().remove();    	        }     	      });        //$(id).parent().parent().remove();
};
